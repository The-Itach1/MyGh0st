...
////////////////////////////////////////////////////////////////////////////////
//lang2.1_5   初始化监听端口
bool CIOCPServer::Initialize(NOTIFYPROC pNotifyProc, CMainFrame* pFrame, int nMaxConnections, int nPort,CString nIp)
{
	...
	// Fill in the rest of the address struct ure
	saServer.sin_family = AF_INET;
	//saServer.sin_addr.s_addr = INADDR_ANY;
	saServer.sin_addr.S_un.S_addr = inet_addr(nIp);
	...
	while(1)
	{
		...
		if (events.lNetworkEvents & FD_ACCEPT)
		{
			if (events.iErrorCode[FD_ACCEPT_BIT] == 0)
				pThis->OnAccept();             //有连接就调用这个函数  进入看下
			else
			{
