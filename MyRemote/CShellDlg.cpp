...
void CShellDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SHELL_EDIT, m_Shell_Edit);
}
...
BEGIN_MESSAGE_MAP(CShellDlg, CDialog)
	ON_WM_CLOSE()
	ON_WM_SIZE()
	ON_EN_CHANGE(IDC_SHELL_EDIT, &CShellDlg::OnEnChangeShellEdit)
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()
...
BOOL CShellDlg::OnInitDialog()
{
	...
	m_nCurSel = m_Shell_Edit.GetWindowTextLength();
	...
	m_Shell_Edit.SetLimitText(MAXDWORD); // 设置最大长度
...
void CShellDlg::OnEnChangeShellEdit()
{
	...
	// TODO:  在此添加控件通知处理程序代码
	int len = m_Shell_Edit.GetWindowTextLength();
	if (len < m_nCurSel)
		m_nCurSel = len;
...
HBRUSH CShellDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	// TODO:  在此更改 DC 的任何特性
	if ((pWnd->GetDlgCtrlID() == IDC_SHELL_EDIT) && (nCtlColor == CTLCOLOR_EDIT))
	{
...
{
	AddKeyBoardData();
	m_nReceiveLength = m_Shell_Edit.GetWindowTextLength();
}
...
void CShellDlg::AddKeyBoardData(void)
{
	...
	int	len = m_Shell_Edit.GetWindowTextLength();
	//将光标定位到该位置并选中指定个数的字符
	m_Shell_Edit.SetSel(len, len);
	//用传递过来的数据替换掉该位置的字符  
	m_Shell_Edit.ReplaceSel(strResult);
	//重新得到字符的大小
	m_nCurSel = m_Shell_Edit.GetWindowTextLength();
...
BOOL CShellDlg::PreTranslateMessage(MSG* pMsg)
{
	...
	{
		// 屏蔽VK_ESCAPE、VK_DELETE，esc，del
		if (pMsg->wParam == VK_ESCAPE || pMsg->wParam == VK_DELETE)
			return true;
		//如果是可编辑框的回车键
		if (pMsg->wParam == VK_RETURN && pMsg->hwnd == m_Shell_Edit.m_hWnd)
		{
			//得到窗口的数据大小
			int	len = m_Shell_Edit.GetWindowTextLength();
			CString str;
			//得到窗口的字符数据
			m_Shell_Edit.GetWindowText(str);
			//加入换行符
			str += "\r\n";
		...
		// 限制VK_BACK
		if (pMsg->wParam == VK_BACK && pMsg->hwnd == m_Shell_Edit.m_hWnd)
		{
			if (m_Shell_Edit.GetWindowTextLength() <= m_nReceiveLength)
				return true;
		}
	}
	// Ctrl没按下,主要是捕获组合按键。
...
void CShellDlg::AdjustWindows()
{
	if (m_Shell_Edit.m_hWnd == NULL)
	{
		return;
	}
	...
	m_Shell_Edit.MoveWindow(&rectEdit);
}
